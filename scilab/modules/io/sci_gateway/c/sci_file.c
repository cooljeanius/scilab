...
static int sci_file_no_rhs(char *fname)
{
    ...
    if (Lhs > 1) /* Types */
    {
        ...
        if (TypeIdsAsString)
        {
            m_out = 1;
            n_out = sizeArray;
            sciErr = createMatrixOfString(pvApiCtx, Rhs + 2, m_out, n_out,
                                          (const char *const *)TypeIdsAsString);
    ...
    if (Lhs > 2) /* names */
    {
        ...
        if (Filenames)
        {
            m_out = 1;
            n_out = sizeArray;
            sciErr = createMatrixOfString(pvApiCtx, Rhs + 3, m_out, n_out,
...
static int sci_file_one_rhs(char *fname)
{
    ...
    if (Lhs > 1) /* Type */
    {
        if (GetFileTypeOpenedInScilab(iID) != 0)
        {
            ...
            n_out = 1;
            sciErr = createMatrixOfString(pvApiCtx, Rhs + 2, m_out, n_out,
                                          (const char *const *)&TypeIdAsString);
            FREE(TypeIdAsString);
            TypeIdAsString = NULL;
        }
        else
