 ...
 * mxArray of size=m*sizeof(double)               */
{
    ...
    return (mxArray *)(intptr_t)C2F(intersci).iwhere[lw - 1];
}
...
mxArray *mxCreateDoubleMatrix (int m, int n, mxComplexity it)
{
    ...
    return (mxArray *)(intptr_t)C2F(vstk).lstk[lw + Top - Rhs - 1];  /* C2F(intersci).iwhere[lw-1]);  */
}
...
mxArray *mxCreateStructArray(int ndim, const int *dims, int nfields, const char **field_names)
{
    ...
    C2F(intersci).ntypes[lw - 1] = AsIs;
    return (mxArray *)(intptr_t)C2F(vstk).lstk[lw + Top - Rhs - 1 ];
}
...
void mxFreeMatrix(mxArray *ptr)
{
    ...
    if ((int)(intptr_t)ptr == C2F(vstk).lstk[Top - Rhs + Nbvars - 1])
    {
...
mxArray *mxCreateLogicalMatrix(int m, int n)
{
    ...
    return (mxArray *)(intptr_t)C2F(intersci).iwhere[new - 1];
}
...
static int mexCallSCI(int nlhs, mxArray **plhs, int nrhs, mxArray **prhs, char *name, int jumpflag)
{
    ...
    {
        for (kk = 1; kk <= nv; ++kk)
            if ((int)(intptr_t)prhs[k - 1] == C2F(vstk).lstk[kk + Top - Rhs - 1])
            {
                break;
            }
        if (kk == nv + 1)
    ...
    for (k = 1; k <= nlhs; ++k)
    {
        plhs[k - 1] = (mxArray *)(intptr_t)C2F(vstk).lstk[nv + k + Top - Rhs - 1];
    }
    Nbvars = nv + nlhs;
    return 0;
}
...
mxArray *mxCreateCharMatrixFromStrings(int m, const char **str)
{
    ...
    C2F(intersci).ntypes[lw - 1] = AsIs;
    return (mxArray *)(intptr_t)C2F(vstk).lstk[lw - 1];
}
...
void clear_mex(int nlhs, mxArray **plhs, int nrhs, mxArray **prhs)
{
    int nv = Nbvars;
    int k;
    int max = (int)(intptr_t)plhs[0];
    for (k = 1; k <= nlhs; k++)
        if ((int)(intptr_t)plhs[k - 1] > max)
        {
            max = (int)(intptr_t)plhs[k - 1];
        }
    for (k = 1; k <= nrhs; k++)
        if ((int)(intptr_t)prhs[k - 1] > max)
        {
            max = (int)(intptr_t)prhs[k - 1];
        }
    for (k = 1; k <= nv; k++)
...
mxArray * C2F(mxcreatestring)(char *string, long int l)
{
    ...
    return (mxArray *)(intptr_t)C2F(vstk).lstk[Top - Rhs + lw - 1];
}
...
{
    int one = 1;
    /* FIXME: is there such thing as a floatptr_t? */
    double *value = ((double *)(intptr_t) * ptr);
...
int C2F(mxcopycomplex16toptr)(double *y, mxArray *ptr, mxArray *pti, int *n)
{
    ...
    /* FIXME: is there such thing as a floatptr_t? */
    double *value = ((double *)(intptr_t) * ptr);
    double *headerm = ((double *)(intptr_t) * pti);
