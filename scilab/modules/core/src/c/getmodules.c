...
static BOOL ReadModulesFile(void)
{
    ...
    ModulesFilename_len = ((strlen(SciPath) + strlen("/")
                            + strlen(basenamemodulesfile) + 1UL)
                           * sizeof(char));
    ModulesFilename = (char *)MALLOC(ModulesFilename_len);
    snprintf(ModulesFilename, ModulesFilename_len, "%s/%s", SciPath,
             basenamemodulesfile);
    FREE(SciPath);
    SciPath = NULL;
...
static BOOL VerifyModule(char *ModuleName)
{
    ...
    FPMN_len = ((strlen(SciPath) + strlen("%s/modules/%s/etc/%s.start")
                 + (strlen(ModuleName) * 2UL) + 1UL) * sizeof(char));
    FullPathModuleName = (char*)MALLOC(FPMN_len);
    snprintf(FullPathModuleName, FPMN_len, "%s/modules/%s/etc/%s.start",
