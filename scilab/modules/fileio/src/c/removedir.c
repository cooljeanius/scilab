...
static int DeleteDirectory(char *refcstrRootDirectory)
{
    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        filename_amt = (sizeof(char) * (strlen(refcstrRootDirectory) + 1UL + strlen(ent->d_name) + 1UL + 1UL));
        filename = MALLOC(filename_amt);
        snprintf(filename, filename_amt, "%s/%s", refcstrRootDirectory,
                 ent->d_name);
        if (isdir(filename))
        {
            /* Delete recursively */
