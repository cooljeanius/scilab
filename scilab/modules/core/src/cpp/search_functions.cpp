...
int upper_char(int id)
{
     ...
     */
    return std::abs(static_cast<int>((id & 0x80) ? (id | 0xffffff00) :  (id & 0xff)));
}
                  ...
                  int* ilp_ptr, int* nn_ptr, int* should_return)
{
    ...
    const int *const lstk_ptr = const_cast<int *>(C2F(vstk).lstk) - 1;
    const int *const istk_ptr = (reinterpret_cast<int *>(C2F(stack).Stk)) - 1;
...
void C2F(sivars)(int* id, int* should_return)
{
    int *const lstk_ptr = const_cast<int *>(C2F(vstk).lstk) - 1;
