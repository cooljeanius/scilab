...
wchar_t *expandPathVariableW(wchar_t *wcstr)
{
    ...
    if (wcstr)
    {
        ...
        for (i = 0; i < NB_ALIAS; i++)
        {
            ...
            {
                wchar_t *wcexpanded1 = getVariableValueDefinedInScilab(VARIABLES_words[i].VariableName);
                if (wcexpanded1)
                {
                    return convertFileSeparators(wcexpanded1);
                }
            }
                    ...
                    if (wcscmp(wcBegin, VARIABLES_words[i].Alias) == 0 )
                    {
                        if ( (wcstr[lenAlias] == L'/') || (wcstr[lenAlias] == L'\\') )
                        {
                            ...
                            if (newBegin)
                            {
                                ...
                                wcexpanded0 = (wchar_t *)MALLOC(sizeof(wchar_t) * (lengthnewBegin + (int)wcslen(&wcstr[lenAlias]) + 1));
                                if (wcexpanded0)
                                {
                                    wcscpy(wcexpanded0, newBegin);
                                    wcscat(wcexpanded0, &wcstr[lenAlias]);
                                    FREE(wcBegin);
                                    wcBegin = NULL;
                                    FREE(newBegin);
                                    newBegin = NULL;
                                    return convertFileSeparators(wcexpanded0);
                                }
                                FREE(newBegin);
                                newBegin = NULL;
                            }
        ...
        wcexpanded0 = (wchar_t*)MALLOC(sizeof(wchar_t) * ((int)wcslen(wcstr) + 1));
        if (wcexpanded0)
        {
            wcscpy(wcexpanded0, wcstr);
            return convertFileSeparators(wcexpanded0);
        }
    }
    return wcexpanded0;
