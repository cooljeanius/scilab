...
static int VCopyObj(const char *fname, int *orig, int *dest, unsigned long fname_length);
...
int C2F(macr2tree)(char *fname, unsigned long fname_len)
{
    ...
    const char *pgrm_tlist[] = {"program", "name", "outputs", "inputs", "statements", "nblines"};
    ...
    int refFlag = 0; /* Flag set if input is a variable reference */
    ...
    if (stkdata[0] <= 0) /* Variable reference */
    {
    ...
    /* Make a copy of input variable to avoid erasing it when writing output */
    ils = il + 1; /* stkdata[ils] = number of outputs */
    ile = ils + nsiz * stkdata[ils] + 1; /* stkdata[ile] = number of outputs */
    ilt = ile + nsiz * stkdata[ile] + 1; /* stkdata[ilt] = codelength */
    ...
    {
        /*
         * Not a reference to variable so we can not get its name from idstk (only used for references)
         * The name is set to "ans" as in fun2string
         */
        strcpy(name[0], "ans");
    }
...
{
    /* try-catch */
    const char *trycatch_tlist[] = {"trycatch", "trystat", "catchstat"};
    int m_trycatch_tlist = 1;
    int n_trycatch_tlist = 3;

    /* if */
    const char *if_tlist[] = {"ifthenelse", "expression", "then", "elseifs", "else"};
    int m_if_tlist = 1;
    int n_if_tlist = 5;

    const char *elseif_tlist[] = {"elseif", "expression", "then"};
    int m_elseif_tlist = 1;
    int n_elseif_tlist = 3;

    /* while */
    const char *while_tlist[] = {"while", "expression", "statements"};
    int m_while_tlist = 1;
    int n_while_tlist = 3;

    /* select */
    const char *select_tlist[] = {"selectcase", "expression", "cases", "else"};
    int m_select_tlist = 1;
    int n_select_tlist = 4;

    const char *case_tlist[] = {"case", "expression", "then"};
    int m_case_tlist = 1;
    int n_case_tlist = 3;

    /* for */
    const char *for_tlist[] = {"for", "expression", "statements"};
    int m_for_tlist = 1;
    int n_for_tlist = 3;
    char **name;
    int namelgth = 0;
...
{
    const char *fun_tlist[] = {"funcall", "rhs", "name", "lhsnb"};
    int m_fun_tlist = 1;
    int n_fun_tlist = 4;
...
{
    const char *eq_tlist[] = {"equal", "expression", "lhs", "endsymbol"};
    int m_eq_tlist = 1;
    int n_eq_tlist = 4;
    ...
    int index2es_pos;
    int nb_index2es = 0;
    const char *op_tlist[] = {"operation", "operands", "operator"};
    int m_op_tlist = 1;
    int n_op_tlist = 3;
    ...
    if (!strncmp(fromwhat, "code29", 6)) /* A code 29 was found in data */
    {
        ...
        for (k = 0; k < nblhs; k++)
        {
            ...
            *index2 = *index2 + nsiz;
            ...
            {
                /* Write tlist items names */
                str2sci((char **)op_tlist, m_op_tlist, n_op_tlist);
...
{
    const char *fun_tlist[] = {"comment", "text"};
    int m_fun_tlist = 1;
    int n_fun_tlist = 2;
...
static int VCopyObj(const char *fname, int *orig, int *dest, unsigned long fname_length)
{
    C2F(vcopyobj)((char *)fname, orig, dest, fname_length);
    Top = *dest;
    return 0;
}
