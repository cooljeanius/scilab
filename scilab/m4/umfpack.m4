...
AC_ARG_WITH([umfpack_library],
	    [AS_HELP_STRING([--with-umfpack-library=DIR],
                            [Set the path to the UMFPACK libraries])],
	    [with_umfpack_library=${withval}],
	    [with_umfpack_library='yes'])dnl

AC_ARG_WITH([umfpack_include],
	    [AS_HELP_STRING([--with-umfpack-include=DIR],
                            [Set the path to the UMFPACK headers])],
	    [with_umfpack_include=${withval}],
	    [with_umfpack_include='yes'])dnl

# Include provided... check if you set it as -Wp,-I/path/ if it can find the
# header
if test "x${with_umfpack_include}" != "xyes"; then
	save_CFLAGS="${CFLAGS}"
	CFLAGS="-Wp,-I${with_umfpack_include}"
	...
	[AC_CHECK_HEADER([umfpack.h],
			...
			[AC_MSG_ERROR([Cannot find headers (umfpack.h) of the library UMFPACK. Please install the dev package (Debian : libsuitesparse-dev), or try configuring with --without-umfpack])
	])
	])
	CFLAGS="${save_CFLAGS}"
fi
...
if test "x${with_umfpack_library}" != "xyes"; then
	...
	AC_CHECK_LIB([amd],[amd_info],
		     [UMFPACK_LIB="-lamd"],
            [AC_MSG_ERROR([libamd: Library missing (Cannot find symbol amd_info). Check if libamd (sparse matrix minimum degree ordering) is installed and if the version is correct, or try configuring with --without-umfpack])])

	AC_CHECK_LIB([umfpack],[umf_divcomplex],
		     [UMFPACK_LIB="-L${with_umfpack_library} -lumfpack ${UMFPACK_LIB}"; UMFPACK_OK=yes],
            [AC_MSG_ERROR([libumfpack : Library missing. (Cannot find umf_divcomplex). Check if libumfpack is installed and if the version is correct (also called lib suitesparse), or try configuring with --without-umfpack])])
...
if test ${UMFPACK_OK} = no; then
	...
	AC_CHECK_LIB([umfpack],[umf_divcomplex],
            ...
            [AC_MSG_ERROR([libumfpack: Library missing. (Cannot find symbol umf_divcomplex). Check if libumfpack is installed and if the version is correct (also called lib suitesparse), or try configuring with --without-umfpack])])
	LIBS="${save_LIBS}"
fi
