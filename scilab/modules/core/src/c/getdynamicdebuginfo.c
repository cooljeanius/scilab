...
char **getDynamicDebugInfo(int *sizeArray)
{
    ...
    *sizeArray = 0;
    ...
    if (meminfo_fd == -1 && (meminfo_fd = open(MEMINFO_FILE, O_RDONLY)) == -1)
    {
        ...
        snprintf(value, value_len, "%u", getfreememory());
        SetDebugMsg(&dynamicDebug[position], "Total free memory", value);
        position++;

        snprintf(value, value_len, "%u", getmemorysize() );
    ...
    else
    {
        ...
        * Get the information
        */
        snprintf(value, value_len,  "%s %s", name.sysname, name.release);
    }
