...
void Fprintf(FILE *f, int indent2, const char *format, ...)
{
    int i;
    static int count = 0;
    va_list ap;
...
void FCprintf(FILE * f, const char *format, ...)
{
    va_list ap;
...
VARPTR VarAlloc(void)
{
    return ((VARPTR) malloc(sizeof(VAR)));
}

BASFUNPTR BasfunAlloc(void)
{
    return ((BASFUNPTR) malloc(sizeof(BASFUN)));
}

FORSUBPTR ForsubAlloc(void)
{
