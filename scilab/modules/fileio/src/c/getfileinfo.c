...
void C2F(getfileinfo)(int *fd, FILE *fa, int *swap2, int *type, int *mode, char *filename, int *lf, int *ierr)
{
    ...
    /* FIXME: the -Wunused-but-set-parameter I fixed here might have actually
     * been telling me to add an extra level of pointer-ness to fa: */
    fa = GetFileOpenedInScilab(*fd);

    if (fa == NULL)
    {
        /* unsure what to do; follow pattern of previous error-looking things */
        *ierr = 3;
        return;
    }
