...
void XMLAttr::setAttributeValue(xmlNode * node, const char *prefix, const char *name, const char *value)
{
    if (node && node->type == XML_ELEMENT_NODE)
    {
        ...
        if (attrs)
        {
            xmlSetNsProp(node, attrs->ns, reinterpret_cast<const xmlChar *>(name),
        ...
        else
        {
            ...
            if (ns)
            {
                xmlSetNsProp(node, ns, reinterpret_cast<const xmlChar *>(name),
                             reinterpret_cast<const xmlChar *>(value));
            }
            else
            {
...
void XMLAttr::setAttributeValue(xmlNode * node, int index, const char *value)
{
    if (node && node->type == XML_ELEMENT_NODE)
    {
        ...
        for (xmlAttr * cur = node->properties; cur; cur = cur->next, i++)
        {
            if (i == index)
            {
                cur->children->content = xmlStrdup(reinterpret_cast<const xmlChar *>(value));
            }
        }
    }
}
...
void XMLAttr::setAttributeValue(xmlNode * node, const char *name, const char *value)
{
    if (node && node->type == XML_ELEMENT_NODE)
    {
        xmlAttr *attrs = xmlHasProp(node, reinterpret_cast<const xmlChar *>(name));
...
const char ** XMLAttr::getNames() const
{
    ...
    for (xmlAttr * cur = node->properties; cur; cur = cur->next, i++)
    {
        arr[i] = reinterpret_cast<char *>(const_cast<xmlChar *>(cur->name));
    }
    arr[size] = 0;
