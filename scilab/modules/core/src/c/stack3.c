...
void CreateCPolyVarFromPtr(int _iNewVal, int **_piVarName, int _iRows, int _iCols,
                           int *_piPow, double *_pdblRealData, double *_pdblImgData)
{
    int iIndex			= 0;
...
int iGetOrient(int _iVal)
{
    ...
    else if (iMode == MTLB_LETTER || iMode == BY_MTLB)
    {
        ...
        /* I did not understand everything about how MtLb works */
        iMode = BY_ALL;
        if (iRows > 1)
        {
            iMode = BY_ROWS;
...
int iAllocComplexMatrixOfDoubleToAddress(int *_piAddr, int _iComplex,
        int _iRows, int _iCols,
        double **_pdblRealData, double **_pdblImgData)
{
#if 0
...
int iGetListItemType(int _iVar, int* _piParentList, int *_piItemNumber, int *_pElemType)
{
    ...
    *_piItemNumber		= piBase[1];
    ...
    /* Warning: variable starts at an even (pair) address, so if the size is odd,
     * we add 1 to have an even address: */
...
int iGetSparseFromAddress(int _iAddr, int* _piRows, int* _piCols, int* _piTotalElem, int* _piElemByRow, int* _piColByRow, int* _piReal, int* _piImg)
{
    ...
    *_piTotalElem		= *istk(_iAddr + 4);
    ...
    /* if ptr are NULL, just return the matrix size */
