...
const char *XMLDocument::getDocumentURL() const
{
    if (document->URL)
    {
        return reinterpret_cast<const char *>(document->URL);
    }
    else
    {
        return "Undefined";
...
void XMLDocument::setDocumentURL(const std::string & url) const
{
    ...
    if (expandedPath)
    {
        xmlFree(static_cast<void *>(const_cast<xmlChar *>(document->URL)));
        newURL = static_cast<char *>(xmlMalloc(sizeof(char *) * (strlen(expandedPath) + 1)));
        memcpy(newURL, expandedPath, sizeof(char) * (strlen(expandedPath) + 1));
        document->URL = reinterpret_cast<xmlChar *>(newURL);
        FREE(expandedPath);
    }
}
...
{
    int size = static_cast<int>(openDocs.size());
...
xmlDoc *XMLDocument::readDocument(const std::string & xmlCode, bool validate, std::string * error)
{
    ...
    doc = xmlCtxtReadDoc(ctxt, reinterpret_cast<const xmlChar *>(xmlCode.c_str()),
                         0, 0, options);
    if (!doc || !ctxt->valid)
    {
        *error = *errorBuffer;
...
xmlParserCtxt *XMLDocument::initContext(std::string * error, bool validate)
{
    ...
    if (validate)
    {
        ctxt->vctxt.error = static_cast<xmlValidityErrorFunc>(errorFunction);
    }
