...
AC_ARG_WITH([fftw_include],
            [AS_HELP_STRING([--with-fftw-include=DIR],
                            [Set the path to the FFTW headers])],
            [with_fftw_include=${withval}],
            [with_fftw_include='yes'])dnl

AC_ARG_WITH([fftw_library],
            [AS_HELP_STRING([--with-fftw-library=DIR],
                            [Set the path to the FFTW libraries])],
            [with_fftw_library=${withval}],
            [with_fftw_library='yes'])dnl

if test "x${with_fftw_include}" != "xyes"; then
	save_CFLAGS="${CFLAGS}"
	CFLAGS="-Wp,-I${with_fftw_include}"
