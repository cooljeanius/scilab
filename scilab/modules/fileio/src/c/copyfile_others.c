...
static int RecursiveCopyDirectory(char *DestinationDir, char *SourceDir)
{
    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        filenameSRC_len = (sizeof(char) * (strlen(SourceDir) + 1UL + strlen(ent->d_name) + 1UL + 1UL));
        filenameSRC = (char*)MALLOC(filenameSRC_len);
        filenameDST_len = (sizeof(char) * (strlen(DestinationDir) + 1UL + strlen(ent->d_name) + 1UL + 1UL));
        filenameDST = (char*)MALLOC(filenameDST_len);

        snprintf(filenameSRC, filenameSRC_len, "%s/%s", SourceDir, ent->d_name);

        if (DestinationDir[strlen(DestinationDir) - 1] == '/')
        {
            snprintf(filenameDST, filenameDST_len, "%s%s", DestinationDir,
                     ent->d_name);
        }
        else
        {
