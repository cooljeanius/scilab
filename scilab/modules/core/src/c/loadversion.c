                      ...
                      int *sci_version_revision)
{
    ...
    if (with_module(modulename))
    {
        ...
        SciPath = getSCIpath();
        len = (strlen(FORMATVERSIONFILENAME) + strlen(SciPath)
               + strlen(modulename) + 1UL);
        f_v_m_len = (sizeof(char) * len);
        filename_VERSION_module = (char *)MALLOC(f_v_m_len);
        snprintf(filename_VERSION_module, f_v_m_len, FORMATVERSIONFILENAME,
                 SciPath, modulename);
        if (SciPath)
        {
            FREE(SciPath);
